...
export default function Initiatives() {
  ...
  // Refs for auto-scrolling logic (unchanged)
  ...
  // Timeline auto-scroll logic (unchanged)
  useEffect(() => {
  ...
  // Draw SVG serpentine path (unchanged)
  useLayoutEffect(() => {
  ...
  // Redraw connectors on viewport resize (unchanged)
  useEffect(() => {
  ...
  return (
    <main className="initiatives-root">
      ...
      {/* Flow heading with extra spacing from above */}
      ...
      {/* Flowchart (horizontal scroll with numbered markers and serpentine path) */}
      ...
      {/* What else can we do for you? section */}
      ...
      <style>{`
        ...
        /* Title and write-up */
        .hero-title{ margin:0; font-family:'Orbitron','Space Grotesk',Poppins,system-ui,sans-serif; font-weight:900; letter-spacing:.6px; font-size:clamp(1.8rem,5vw,3rem); line-height:1.05; position:relative; }
        .reveal{ display:inline-block; position:relative; overflow:hidden; }
        .reveal::before{ content:""; position:absolute; inset:0; background: linear-gradient(115deg, transparent 0%, rgba(127,247,234,.12) 18%, transparent 42%); transform: translateX(-130%); filter: blur(1px); animation: sweep 1200ms ease-out 180ms forwards; mix-blend-mode: screen; }
        ...
        .subhead{
          ...
          font-weight:700; letter-spacing:.3px; font-size:clamp(1.1rem, 3vw, 1.3rem); color:var(--fg);
          position:relative; width:max-content;
        }

        /* Line under subheadings */
        .subhead-line::after{
        ...
        /* This allows the inline style 'display: flex' on the JSX element to work correctly. */
        .flow-steps{
          position:relative;
          gap: clamp(28px, 4.5vh, 44px);
          overflow-x: auto;
        ...
        .flow-row .badge{
          ...
          background: radial-gradient(circle at 40% 40%, #7ff7ea, #12e7cf);
          color:#012326; font-weight:900; font-family:'Orbitron','Space Grotesk',Poppins,system-ui,sans-serif;
          display:flex; align-items:center; justify-content:center; box-shadow:0 10px 22px rgba(0,0,0,.35);
        }
        .row-content h3{
          margin:0 0 6px; font-weight:800; font-family:'Orbitron','Space Grotesk',Poppins,system-ui,sans-serif;
          letter-spacing:.2px; font-size: clamp(1rem, 2.3vw, 1.1rem); color:var(--fg);
        }
        .row-content p{
          margin:0; color: var(--muted); line-height:1.7; max-width: 980px;
        }
        ...
        .prog-card:not(:last-child) { margin-bottom: 0; } /* Removed explicit margin-bottom as grid gap handles it */
        .prog-head{ flex:1 1 100%; display:flex; flex-direction:column; gap:8px; }
        .prog-head h3{ margin:0; font-family:'Orbitron','Space Grotesk',Poppins,system-ui,sans-serif; font-weight:800; letter-spacing:.2px; font-size: clamp(1rem, 2.4vw, 1.25rem); color:var(--fg); }
